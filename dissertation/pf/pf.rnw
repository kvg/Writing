...
To resolve these issues, we implemented a filtering solution for novel kmers in three parts.  First, we implemented a manual procedure for specifying coverage limits, selecting a lower limit of $10x$ and an upper limit of $74x$.  Next, we implemented software to explore the trio graph starting at confident novel kmers rejected by the contamination filter using our condition-limited depth first search software.  Explored branches containing novel kmers that are on the same branch as rejected kmers are similarly tagged for rejection.  Finally, we explore the trio graph starting with remaining confident novel kmers.  If the explored subgraph does not connect with the parental graphs in any way, the kmers in the branch are marked for rejection and removed from the graph.  Any novel kmer surviving this battery of tests is considered "filtered".

Application of these filters brings our apparent novel kmer count down from $28,924$ to $210$ accepted, $28,714$ rejected ($429$ rejected for coverage, $17,666$ for contamination, $10,619$ orphaned).  We compared these lists of accepted and rejected novel kmers to the PacBio data.  To mitigate errors in the assembly, we required that a "true positive novel kmer" be present in both the PacBio dataset and the Illumina data.  We found $48$ true positives and $162$ false positives.  Examining the rejected kmers, we found $28,691$ to be true negatives and $23$ false negatives.  Our computed sensitivity and specificity are $68\%$ and $99\%$, respectively.  All false negatives are due to low coverage; each kmer in question has coverage less than $3x$, and the vast majority have coverage of $1x$.  While some false-positives appear to be due to cryptic contamination that we have been unable to resolve, a handful manifest as seemingly heterozygous SNPs (Figure \ref{fig:fp}).  As the genome in question is haploid, such events can represent regions of elevated copy number and subsequent divergence of one of the copies, or it can represent cross-contamination between 803xGB4 samples.  The inability to rediscover these kmers in the PacBio dataset implies the latter.  It's possible that this event is actually segregating in the Illumina sample, but that the sample is a mixture of at least two parasites.

\begin{figure}[h!]
  \centering
    \includegraphics[width=\textwidth]{fpcomp}
  \caption{A false-positive event.  a. Local subgraph at the false-positive site, wherein the child's graph contains paths that traverse the series of novel kmers as well as the parental kmers.  b. IGV screenshot of the site showing reads and assembly information for the trio, aligned to the PacBio PG0466-C assembly.  Top panel: 803.  Middle two: GB4 reads and PacBio assembly (sliced to facilitate alignment).  Lower two: PG0446-C reads and contig containing the variant in question from the a. panel.}
  \label{fig:fp}
\end{figure}
...
The 803xGB4 samples have consistent novel kmer behavior with the samples from other crosses, despite the concern regarding assembly lengths being substantially longer than expectation.
...
\end{subfigure}
\caption{Left: number of novel kmers found in each sample, broken down into "trusted", "confident", and "all" categories.  Right: number of novel contigs (contigs containing at least one trusted novel kmer) per sample.}
\label{fig:novelty}
\end{figure}
