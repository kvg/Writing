...
Finally, note that this algorithm could fail to produce a correct FASTA file if there are multiple variants that overlap.  We are careful to avoid that scenario; we set our simulated variants to be placed no closer than $1000$ bp from one another.
...
Generating the genomes of the children is slightly more involved, as there is much more biology to simulate, and many more considerations to be made when placing variants. We generated VCF descriptions of the children using a multi-stage pipeline that simulates (in order):

\begin{enumerate}
    \item homologous recombination between 3D7 and HB3
    \item gene conversions
    \item NAHR events between \textit{var} genes
    \item \textit{de novo} SNPs, insertions, deletions, and inversions
\end{enumerate}

\subsection{Homologous recombination}

\subsubsection{Allelic}
...
Recombination frequency will be dependent on the length of the chromosome. The empirical distributions were generated from the $75$ samples in the crosses data. These are plotted in Figure \ref{fig:recomb-1}. We simulated the number of recombination events per chromosome by drawing a random number from these distributions. The recombination sites themselves were chosen by drawing a uniform random variate between $1$ and the length of the chromosome. Although there are certainly hotspots and coldspots of recombination in the genome, we have ignored this complication.  An example haplotype mosaics for chromosome $12$ is found in Figure \ref{fig:haplotypes12}.

\begin{figure}[h!]
  \centering
    \includegraphics[width=\textwidth]{haplotypes12}
  \caption{Simulated haplotype mosaics for chromosome 12.}
...
\subsubsection{Non-allelic}

NAHR events were generated by first finding compatible recombination partners. This list was generated by determining which 3D7 \textit{var} genes had been transmitted to the child, grouped by ups class and telomeric positioning ($5'$ or $3'$). In each group, two random genes were chosen. If necessary, the gene sequences were reverse complemented to have matching orientation. As NAHR events between two \textit{var} genes appear to occur in regions of homology, we identified shared $9$-mers, positioned between $20$ bp and $100$ bp between the two genes, to act as possible recombination sites. We selected between $2$ and $5$ random switches between the sequences and copied sequence data accordingly. Keeping in mind that our previous work has shown that the recombined \textit{var} genes are lost in order to produce the chimera, we added VCF records to delete the previous \textit{var} alleles from the child's genome and replace one of them with the recombined sequence.
...
To simulate de novo SNPs, we added $250$ sites with random (non-reference) alleles at random positions throughout the genome.  We also simulated $10$ insertion, deletion, and inversion events at every length between $1$ and $100$ bp. For insertions, random alleles were generated and tested to ensure they did not match the reference sequence. For deletions, we simply replaced the reference allele with a truncated allele of the prescribed length. For inversions, we replace the reference allele with its complement.

\subsubsection{Expansion of short tandem repeats}

\begin{figure}[h!]
  \centering
    \includegraphics[width=\textwidth]{str_exp_and_con}
  \caption{Expansion and contraction of short tandem repeats (STRs)}
  \label{fig:str_exp_and_con}
\end{figure}

As a special case of indels, we sought specifically to simulate the expansion and contraction of short tandem repeats (STRs), depicted in Figure \ref{fig:str_exp_and_con}. As these are constrained to occur at loci where there are already existing repeats, we first built a map of repeated $2$-bp, $3$-bp, $4$-bp, and $5$-bp sequences in the 3D7 genome. We filtered these lists, retaining only STRs where the repeated unit occurred at least three times. For each simulated event, we randomly chose a position from the appropriate list and select a number of units to add or remove. The number of units is constrained to be less than the length of the number of repeat units of the existing STR. With these considerations, we simulated $10$ expansions and $10$ contractions at the aforementioned repeat unit sizes.

\subsubsection{Tandem duplications}

For tandem duplications, we simulated $10$ events at each length between $10$ bp and $50$ bp by selecting sequences randomly from the genome and inserted an identical copy at the same locus.
...
Construction of a coverage profile consists of two sub-problems: providing a complete description of where reads fall on the existing reference sequence, and tranferring that information sensibly to the modified reference sequence.  We address each of these needs with custom tools.

\subsubsection{ComputeBaseAndFragmentErrorRates}

The ComputeBaseAndFragmentErrorRates tool provides a precise specification for the number of fragments and reads found starting at of every position in the canonical reference genome, as well as an accounting as to which reads and fragments contained any kind of error (mismatch or indels).  Briefly, we iterate over every chromosome in the reference genome, advancing through every aligned read stored in an exemplar sample's BAM file.  We instantiate a chromosome-length array indicating the number of reads that start at a given position and the number of reads that contain apparent errors (any discrepancy from the reference sequence).

We must also store information about read fragment starts and error rates, requiring us to keep track of read pairs as we traverse the BAM file.  To do so, we hash the read name to the first instance of the read we see along the length of a chromosome.  As paired-end reads have the same name, the second time we see the same read name, we will have found the second end of the pair.  We then increment the count at the chromosome array position that corresponds to the $5'$-most end of the pair, and increment the fragment errors array based on errors in either end of the pair.
...
\Function{ComputeBaseAndFragmentErrorRates}{BAM}
    \For{$\textit{chr} \textrm{ in } \textit{chrs}$}
        \State $\textit{numReadsWithErrors} \gets []$
        \State $\textit{numReads} \gets []$
        \State $\textit{numFragmentsWithErrors} \gets []$
        \State $\textit{numFragments} \gets []$
        \State $\textit{seenReads} \gets []$

        \State $\textit{reads} \gets \textit{BAM.getAllReads(chr)}$

        \For{$\textit{read} \textrm{ in } \textit{reads}$}
            \State $\textit{readErrorPositions} \gets \textit{getPositionsWithErrors(read)}$
            \State $\textit{refErrorPositions} \gets \textit{convertReadPositionsToReferencePositions(readErrorPositions)}$

            \For{$\textit{refErrorPosition} \textrm{ in } \textit{refErrorPositions}$}
                \State $\textit{numReadsWithErrors[refErrorPosition]}++$
            \EndFor

            \For{$\textit{readPosition} \textrm{ in } 1:\textit{read.length()}$}
                \State $\textit{numReads[convertReadPositionsToReferencePositions(readPosition)]}++$
            \EndFor

            \If{$\textit{!seenReads.contains(read.getName())}$}
                \State $\textit{seenReads} \gets \textit{read.getName()}$
            \Else
                \State $\textit{mateErrorPositions} \gets \textit{getPositionsWithErrors(seenReads[read.getName()])}$

                \If $\textit{readErrorPositions.size()} + \textit{mateErrorPositions.size()} \ge 1$
                    \State $\textit{numFragmentsWithErrors[seenReads[read.getName()].getAlignmentStart()}++$  
                \EndIf

                \State $\textit{numFragments[seenReads[read.getName()]}++$  
            \EndIf
        \EndFor

        \For{$\textit{pos} \textrm{ in } 1:\textit{numReads.length()}$}
            \State $\textit{print chr, pos, numReadsWithErrors[pos], numReads[pos], numFragmentsWithErrors[pos], numFragments[pos]}$
        \EndFor
    \EndFor
\EndFunction
\end{algorithmic}
...
\subsubsection{LiftoverFromRefToChild}

The genomic coordinates present in the table produced by Algorithm \ref{alg:ComputeBaseAndFragmentErrorRates} must be transformed from the reference sequence to the simulated genome before it can be used.  To do so, we developed a tool that could liftover the coordinates appropriately when given the table, reference sequence, and a VCF file describing the alterations made to the reference to transform it into the simulated genome.  This is accomplished as follows: we first iterate through each chromosome and store the entire sequence as an array of strings.  Each string is of variable length, and at this stage of the construction, all stored strings are of length $1$: the length of the reference allele present at that location.  At positions where a variant is present, we make alterations to the array.  We first replace reference nucleotide strings with zero-length strings up to the length of the reference allele.  We then set the string at the variant position to the alternate allele indicated in the VCF file.  Thus, we are able to handle SNPs (one reference base blanked out, then replaced with one alternate base), insertions (one base blanked out, many replaced), deletions (many bases blanked out, one replaced), and MNPs (many bases blanked out, many replaced).  When all of the strings are concatenated together, the new chromosome is constructed.

Once the chromosome is generated, we advance through the sequence one position at a time.  At each position, we emit the contents of the previously constructed table with the number of reads, fragments, and errors for each.  At some positions, the presence of variants will have modified the underlying sequence such that new positions do not have explicit read and fragment information to lift over.  To account for this, we keep running statistics on previously seen read and fragment statistics from which we can compute running means and standard deviation.  We generate new values for the number of reads, fragments, and error rates as necessary as the mean plus standard deviation of the relevant metric, ensuring the values are never less than $0$, and that we round up or down to the nearest integer (error rates are rounded up and read/fragment counts are rounded down to increase our self-penalty).
...
\begin{enumerate}
    ...
    \item number of chimeric pairs (paired-end reads with one end aligned to a different chromosome than the other)
    \item number of mismatches 
    \item number of insertions
    \item number of deletions
\end{enumerate}

Note that some of the mismatches, insertions, and deletions may represent true variation between the sample and the reference sequence.  We do not mask these sites out.  While this increases the apparent error rate, the variant rates are typically low compared to the number of bases in the reference sequence itself, making the difference negligible.  Furthermore, we will choose the reference sample as the exemplar dataset for the read simulations.  Since this sample should theoretically be identical to the reference sequence, this choice obviates the need for any masking of variants in the reference sample against the reference sequence.
...
Note that these distributions are not conditioned on position in the read.
