...
\newthought{We now present a novel graphical method} for detecting and classifiying \textit{de novo} variants.  We shall present some brief foundational background on \textit{de novo} assembly, upon which the variant caller is built.  The simulation framework and dataset we described in Chapter \ref{ch:simulation} will be used to establish the method's accuracy.
